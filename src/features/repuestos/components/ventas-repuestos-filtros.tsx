...
export function VentasRepuestosFiltros() {
  ...
  function buildUrl(overrides: Record<string, string>) {
    ...
    return `/repuestos/ventas?${p.toString()}`;
  }
  ...
  function handleClear() {
    ...
    router.push(`/repuestos/ventas?${p.toString()}`);
  }
  ...
  return (
    <div className="flex flex-wrap items-center gap-2">
      ...
      <div className="flex items-center gap-1.5">
        ...
        <input type="date" value={desde} onChange={(e) => handleDateChange("desde", e.target.value)}
      ...
      <div className="flex items-center gap-1.5">
        ...
        <input type="date" value={hasta} onChange={(e) => handleDateChange("hasta", e.target.value)}
          className="h-9 rounded-lg border border-input bg-transparent px-3 text-sm outline-none focus:border-ring focus:ring-2 focus:ring-ring/30" />
      </div>
      {hasFilters && (
